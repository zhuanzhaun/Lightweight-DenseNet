...
class AnchorsGenerator(nn.Module):
    ...
    def  __init__(self, sizes=(128, 256), aspect_ratios=(0.5, 1.0, 2.0)):
...
class RPNHead(nn.Module):
    ...
    def forward(self, x):
        ...
        # 生成backbone的特征图
            # feature_maps = feature.squeeze(0).permute(1, 2, 0).cpu()
            #
            # # 创建一个36张子图的画布
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                # 计算anchors与真实bbox的iou信息（分批计算，提升速度并避免显存溢出）
                match_quality_matrix = box_ops.box_iou_chunked(gt_boxes, anchors_per_image, chunk_size=10000)
                match_quality_matrix = match_quality_matrix.to(anchors_per_image.device)
                # top_values, top_indices = torch.topk(match_quality_matrix, 1, dim=0)
                # top_values1, top_indices1 = torch.topk(top_values, 1, dim=-1)
                ...
                # top_values1, top_indices1 = torch.topk(labels_per_image, 1, dim=-1)
    ...
    def _get_top_n_idx(self, objectness, num_anchors_per_level):
        ...
        for ob in objectness.split(num_anchors_per_level, 1):
            ...
            # top_level是前num个预测分数的数值，top_n_idx是这些分数对应的索引
            top_level, top_n_idx = ob.topk(pre_nms_top_n, dim=1)
            # ("当前图像前景预测最大值:" , top_level[0,0],"后续会走一个sigmoid" )
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        # 原始张量的最小值和最大值
        min_x = objectness.min()
        max_x = objectness.max()

        # 线性放缩
        objectness = (b - a) * (objectness - min_x) / (max_x - min_x) + a
        # 计算目标预测概率损失
    ...
    def forward(self,
        ...
        # 可视化RPNHead特征值,显示不出区别
        # image_array = objectness[0][0].squeeze().cpu().detach().numpy()
        # image_array = image_array*2560000
