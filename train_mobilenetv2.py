...
def create_model(num_classes):
    ...
    backbone = Steel(weights_path="./backbone/Steel_Model.pth")
    backbone = create_feature_extractor(backbone, return_nodes={"relu23": "0"})
    backbone.out_channels = 24  # 设置对应backbone输出特征矩阵的channels
    anchor_generator = AnchorsGenerator(sizes=((14),),
                                        aspect_ratios=((1.0),))

    roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'],  # 在哪些特征层上进行roi pooling
                                                    output_size=[7, 7],   # roi_pooling输出特征矩阵尺寸,实际池化的是bbox
                                                    sampling_ratio=2)  # 插值时的采样率
...
def main():
    ...
    VOC_root = "./"  # VOCdevkit
    aspect_ratio_group_factor = 1  # 宽高比相似的锚框被分为三组
    batch_size = 1
    ...
    # create model num_classes equal 二分类
